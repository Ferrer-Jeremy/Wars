                ...
                if (boucle == true)
                {
                    ...
                    for (int j = 0; j < zone.GetLength(0); j++)    //GetLength(0) recupere la longeur de la premiere coordonnée de l'array
                    {
                        if (zone[j, 0] != 0 || zone[j, 1] != 0)// si l'un des 2 est different de 0
                        {
                            ...
                            {
                                if (zone[k, 0] != 0 || zone[k, 1] != 0)// si l'un des 2 est different de 0
                                {
                                    if (zone[j, 0] == zone[k, 0] && zone[j, 1] == zone[k, 1]) //si on a 2 fois le meme chiffre (sauf 0,0) on met le 2eme a zero
                                    {
                                        zone[k, 0] = 0;
                                        zone[k, 1] = 0;
                                    }
